...
const uploadToCloudinary = (file) =>
  new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { folder: "news-app", resource_type: "image" },
      (error, result) => {
...
router.put("/:id", protect, upload.array("images", 10), async (req, res) => {
  try {
    ...
    const imageFiles = req.files || [];
    const uploaded = await Promise.all(imageFiles.map(uploadToCloudinary));
    const newImages = uploaded.map((result, i) => ({
      url: result.secure_url,
      caption: req.body[`caption_${i}`] || "",
    }));
    let keptImages = article.images;
    if (req.body.existingImages) {
      keptImages = JSON.parse(req.body.existingImages);
    }
    if (req.body.removedImages) {
      const removed = JSON.parse(req.body.removedImages);
      keptImages = keptImages.filter((img) => !removed.includes(img.url));
    }
    ...
    article.coverImage = allImages.length > 0 ? allImages[0].url : null;
